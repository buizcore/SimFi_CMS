...
class DbPostgresql extends Db_Connection
{
     ...
     * @param string $schema            
     */
    public function __construct($console, $dbName, $user, $passwd, $host = '127.0.0.1', $port = '5432', $schema = 'public')
    {
        $this->dbName = $dbName;
        $this->user = $user;
        $this->passwd = $passwd;
    ...
    public function setSearchPath($schema)
    {
        ...
        if (! $result = pg_query($this->connection, $sqlstring)) {
            ...
            throw new DbException('Query '.$sqlstring.' Failed: '.pg_last_error($this->connection));
        }
...
// //////////////////////////////////////////////////////////////////////////////
    ...
    public function select($sql, $singleRow = null, $expectResult = false)
    {
        ...
        if (! $result = pg_query($this->connection, $sql)) {
            // Fehlermeldung raus und gleich mal nen Trace laufen lassen
            throw new DbException('Query '.$sql.' Failed: '.pg_last_error($this->connection));
        }
    ...
    public function insert($sql, $tableName = null, $tablePk = null)
    {
        ...
        if (! $result = pg_query($this->connection, $sqlstring)) {
            throw new DbException('Failed to receive a new id: '.pg_last_error($this->connection));
        }
...
// //////////////////////////////////////////////////////////////////////////////
    ...
    public function nextVal($seqName)
    {
        ...
        if (! $result = pg_query($this->connection, $sqlstring)) {
            throw new DbException('No Db Result: '.pg_last_error($this->connection).' '.$sqlstring);
        }
...
// //////////////////////////////////////////////////////////////////////////////
    ...
    public function begin()
    {
        ...
        if (! $result = pg_query($this->connection, 'BEGIN')) {
            throw new DbException('Fehler beim ausführen von Commit: '.pg_last_error($this->connection));
        }
