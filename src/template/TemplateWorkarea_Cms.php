...
class TemplateWorkarea_Cms extends TemplateWorkarea
{
  ...
  public function getVal($key, $subKey = null)
  {
    ...
    if(is_null($subKey)){
      return @!is_null($this->values[$key])
      ? $this->values[$key]
      : null;
    }
  ...
  public function cmsLink($key, $ssl = false, $lang=null)
  {
    ...
    if($ssl){
      ...
      return isset($this->links[$key])
        ? $this->conf->ssl_base_url.$lang.'/'.$this->links[$key]
        : $this->conf->ssl_base_url.$lang.'/'.$key;

    } else {

      return isset($this->links[$key])
        ? $this->conf->base_url.$lang.'/'.$this->links[$key]
        : $this->conf->base_url.$lang.'/'.$key;
    }
