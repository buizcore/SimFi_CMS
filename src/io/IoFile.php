...
class IoFile
{
     ...
     *
     * @param boolean $full
     *            Soll der Ordnerpfad mit ausgegeben werden
     * @return string
     */
    ...
    {
        $ending = substr($this->fileName, strrpos($this->fileName, '.'));
        
        if (isset(IoMime::$text[$ending])) {
            return IoMime::$text[$ending];
        } else {
     ...
     *
     * @param
     *            string Group Name der Gruppe
     * @return boolean
     */
    public function setGroup($group)
    {
        if (is_string($group) and chgrp($this->folder . '/' . $this->fileName, $group)) {
    ...
    } // end public function getTimeCreated */
    
    /**
     * Abfragen der aktuellen Dateigröße
     *
     * @param
     *            string Format Das Rückgabeformat
     * @return int
     */
    ...
    public function copy($target)
    {
        ...
        if (is_writeable($data['dirname'])) {
            ...
            throw new SimFiException('target folder for copy is not writeable: ' . $data['folder']);
        }
    ...
    public function delete()
    {
        if (is_writeable($this->folder . '/' . $this->filename)) {
            ...
            throw new SimFiException('no enough rights to delete: ' . $this->folder . '/' . $this->filename);
            return false;
        }
